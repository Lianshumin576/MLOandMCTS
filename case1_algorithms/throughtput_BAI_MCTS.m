...
for iteration = 1:mc,iteration
    ...
    fixloc = [];
    Random = [];
    root1 = StateNode_BAI(0, 0, 0, sum(num_sta)-length(fixloc), length(arm_space_singlesta), 0.1/(sum(num_sta)-length(fixloc)), 1-(0.9)^(1/(sum(num_sta)-length(fixloc)))); %  创建根节点 0.05*0.4=0.02
    root1.travel_time = 1;
    arm = [];
 for T = 1:Horizon,T
...
G1 = sm_Real_carrier_sensing_graph_HD_plus(x_band1, y_band1, TP_temp, CST_temp, alpha, G_0_1, length(y_band1), index_band1);
G2 = sm_Real_carrier_sensing_graph_HD_plus(x_band2, y_band2, TP_temp, CST_temp, alpha, G_0_2, length(y_band2), index_band2);
G3 = sm_Real_carrier_sensing_graph_HD_plus(x_band3, y_band3, TP_temp, CST_temp, alpha, G_0_3, length(y_band3), index_band3);
...
Throughput1 = sm_CSMAsimMarkov_ATP_v2_update(xap, x_band1, yap, y_band1, num_choose1,  G1, TP_temp, SNR_index1, Tran_rate, rho1, d_max, Omega, G_0_1, alpha, sigma2_n, length(y_band1), index_band1);
Throughput2 = sm_CSMAsimMarkov_ATP_v2_update(xap, x_band2, yap, y_band2, num_choose2,  G2, TP_temp, SNR_index2, Tran_rate, rho2, d_max, Omega, G_0_2, alpha, sigma2_n, length(y_band2), index_band2);
Throughput3 = sm_CSMAsimMarkov_ATP_v2_update(xap, x_band3, yap, y_band3, num_choose3,  G3, TP_temp, SNR_index3, Tran_rate, rho3, d_max, Omega, G_0_3, alpha, sigma2_n, length(y_band3), index_band3);
...
root1.backpropogate(terminal_node1, throughput/0.3);% /0.3 is  for normalizing
...
record_arm(T,:) = arm_index;
 ...
 Record_net_level_pool(loop,:) = mean(Record_net_level);
 Record_ap1_sta1_level_pool(loop,:) = mean(Record_ap1_sta1_level);
 Record_ap1_sta2_level_pool(loop,:) = mean(Record_ap1_sta2_level);
 Record_ap2_sta1_level_pool(loop,:) = mean(Record_ap2_sta1_level);
 Record_ap2_sta2_level_pool(loop,:) = mean(Record_ap2_sta2_level);
 Record_ap3_sta1_level_pool(loop,:) = mean(Record_ap3_sta1_level);
 Record_ap3_sta2_level_pool(loop,:) = mean(Record_ap3_sta2_level);
 Record_arm_pool(loop,:,:) = Record_arm(end,:,:);
end
 Record_net_level_mean = mean(Record_net_level_pool);
 Record_ap1_sta1_level_mean = mean(Record_ap1_sta1_level_pool);
 Record_ap1_sta2_level_mean = mean(Record_ap1_sta2_level_pool);
 Record_ap2_sta1_level_mean = mean(Record_ap2_sta1_level_pool);
 Record_ap2_sta2_level_mean = mean(Record_ap2_sta2_level_pool);
 Record_ap3_sta1_level_mean = mean(Record_ap3_sta1_level_pool);
 Record_ap3_sta2_level_mean = mean(Record_ap3_sta2_level_pool);
save BAI_mc1000_T2000_rng1 Record_arm_pool Record_net_level_mean Record_ap1_sta1_level_mean Record_ap1_sta2_level_mean Record_ap2_sta1_level_mean Record_ap2_sta2_level_mean Record_ap3_sta1_level_mean Record_ap3_sta2_level_mean
