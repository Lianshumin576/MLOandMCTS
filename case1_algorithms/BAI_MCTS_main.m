...
for iteration = 1:mc,iteration
    ...
    fixloc = [];%for LLM
    fixconf = [];
    root1 = StateNode_BAI(0, 0, 0, sum(num_sta)-length(fixloc), length(arm_space_singlesta), 0.02/(5e8)/(sum(num_sta)-length(fixloc)), 1-(1-0.1)^(1/(sum(num_sta)-length(fixloc)))); %  5e8 is for normalization
    root1.travel_time = 1;
    arm = [];
 for T = 1:Horizon,T
[root1, terminal_node1, arm_index] = mcts_plus(root1, sum(num_sta), length(arm_space_singlesta), fixloc, fixconf); 
...
%2.using the V.D version (or discrete event simulator) 
...
root1.backpropogate(terminal_node1, throughput/5e8);% /5e8 is  for normalization
